...
// Recording state (DEPRECATED - migrating to DeckInput request/response pattern)
// Recording is now controlled via:
//   - input.record_start/record_stop: One-shot requests
//   - AudioHardware::is_recording(): Query current state
//   - input.source: PlaybackSource::File or PlaybackSource::Loop
struct RecordingState {
    bool requested = false;     // DEPRECATED: Use input.record_start/stop
    bool active = false;        // DEPRECATED: Use AudioHardware::is_recording()
    bool use_loop = false;      // DEPRECATED: Use input.source
...
// Audio feedback state (DEPRECATED - migrating to DeckInput/DeckProcessingState)
// Feedback is now controlled via:
//   - input.beep_request: One-shot request (BeepType enum in deck_input.h)
//   - DeckProcessingState::current_beep/beep_position: Audio engine state
struct FeedbackState {
    ...
    int beep_type = NONE;           // DEPRECATED: Use input.beep_request
    unsigned long beep_position = 0; // DEPRECATED: Managed by audio engine
